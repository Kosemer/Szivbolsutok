...
const ResizeImage = ({ imageFile, setImageFile, onResizeSuccess, scale = 0.7, selectedFile }) => {
    ...
    img.onload = () => {
      ...
      canvas.toBlob(
        (blob) => {
          ...
          onResizeSuccess(webpFile.size);
        },
        "image/webp",
        0.9 // Csökkentett minőség a fájlméret optimalizálásához
      );
    };
  };
  ...
  return (
    <div className={cartCtx.isOpenResize ? classes.container : classes.containerClosed}>
      ...
      <p className={classes.description}>
        A weboldalon lévő képek optimalizáltak, ami azt jelenti, hogy az eredeti méretükhöz képest 75%-al csökkentve vannak.
        Ez jelentős méretbeni különbséget jelent, ami a minőségben nem vehető észre.
        <strong>
          <br />Javasolt a képek optimalizálása feltöltés előtt, amennyiben nagyobb méretűek, hogy a weboldal betöltési ideje rövid legyen.
        </strong>
      </p>
      <div className={classes.resizeButtonContainer}>
        <button type="button" onClick={resizeImage} disabled={!imageFile || imageFile.resized} className={classes.resizeButton}>
          Kép átméretezése {scale * 100}%-ra
        </button>
      </div>
      {!selectedFile && buttonClicked && <p className={classes.errorMessage}>Válassz ki egy képet</p>}
    </div>
  );
};
